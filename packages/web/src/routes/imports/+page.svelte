<script lang="ts">
  ...
  const columns: ColumnDef<ImportListItem, any>[] = [
    {
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return val ? val.replace(/_/g, ' ').replace(/^\w/, (c: string) => c.toUpperCase()) : '\u2014';
      },
    },
    {
      accessorKey: 'status',
      header: 'Status',
      cell: ({ row }) =>
        renderComponent(ImportStatusBadge, { status: row.original.status }),
    },
    {
    ...
    },
    {
      ...
      header: '',
      cell: ({ row }) =>
        renderComponent(ImportViewLink, { importId: row.original.importId }),
      enableSorting: false,
    },
  ];
</script>
...
<div class="space-y-6">
  <div>
    <h2 class="text-2xl font-bold text-ink">Imports</h2>
    <p class="mt-1 text-muted">Upload normalised credit files and view ingestion history.</p>
  </div>
